...
export class CreateCustomerUseCase {
  ...
  async execute(organizationId: string, input: CreateCustomerInput): Promise<Customer> {
    if (input.document) {
      const existing = await this.customerRepository.findByDocument(organizationId, input.document);
      if (existing) {
