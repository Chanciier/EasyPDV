...
export class UpdateCustomerUseCase {
  ...
  async execute(organizationId: string, id: string, input: UpdateCustomerInput): Promise<Customer> {
    const existing = await this.customerRepository.findById(organizationId, id);
    if (!existing) {
      throw new CustomerNotFoundError(id);
    }
    if (input.document && input.document !== existing.document) {
      const conflicting = await this.customerRepository.findByDocument(organizationId, input.document);
      if (conflicting) {
        throw new DocumentAlreadyInUseError(input.document);
      }
    }
    return this.customerRepository.update(organizationId, id, input);
  }
}
