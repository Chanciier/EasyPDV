...
import { formatBRL, type PaymentMethod } from '@/lib/pos-data'
...
const METHODS: { key: PaymentMethod; label: string; icon: typeof Banknote; num: string }[] = [
  ...
  { key: 'cartao_credito', label: 'Crédito', icon: CreditCard, num: '2' },
  { key: 'cartao_debito', label: 'Débito', icon: Landmark, num: '3' },
...
export function PaymentDialog({
  ...
  const canConfirm = !isCash || receivedNum >= total
            ...
            {METHODS.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 rounded-lg border-2 px-2 py-3 text-sm font-medium transition-colors ${
                    active
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="0,00"
                className="w-full rounded-lg border border-input bg-background px-4 py-3 font-mono text-2xl font-semibold outline-none focus:border-primary focus:ring-2 focus:ring-ring/40"
              />
            </div>
            <div className="flex flex-wrap gap-2">
              {quickValues.map((v, i) => (
                <button
                  ...
                  className="rounded-lg border border-border bg-secondary px-3 py-1.5 font-mono text-sm font-medium transition-colors hover:bg-muted"
                >
