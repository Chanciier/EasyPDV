...
/**
 * Ação explícita do operador (botão "Emitir nova NFC-e" no Histórico, só
 * aparece pra NFC-e "error") — mesma lógica de RetryFiscalDocumentUseCase:
 * não cai pro cache local em caso de falha, o operador precisa saber que a
 * reemissão não deu certo.
 */
@Injectable()
export class ReissueFiscalDocumentUseCase {
  ...
  async execute(saleId: string): Promise<FiscalDocument | null> {
    const remote = await this.fiscalGateway.reissueManually(saleId);
    if (!remote) {
      return null;
    }
