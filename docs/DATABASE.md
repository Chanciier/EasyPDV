...
| FiscalDocument | Espelho local do status fiscal da venda | **Implementado na Sprint 12.** type: nfce \| comprovante_nao_fiscal; status: pending \| issued \| cancelled \| error. `@@unique` por `saleId` — uma linha por venda. Nunca escrito localmente por conta própria: `GetFiscalStatusUseCase` busca ao vivo em `GET /fiscal/sale/:saleId` no Intermediador e faz upsert aqui (`upsertFromRemote`), pra ficar visível no Histórico mesmo se a rede cair depois. Se a busca falhar (Intermediador fora do ar), cai pro que já está espelhado em vez de propagar o erro |
...
| FiscalDocument | NFC-e emitida no Bling a partir do pedido de venda | **Implementado na Sprint 12.** `@@unique([organizationId, provider, saleId])`. Gerada via `POST /pedidos/vendas/:id/gerar-nfce` (rascunho) + `POST /nfce/:id/enviar` (transmissão real pra SEFAZ) — opt-in via `BLING_NFCE_AUTO_EMIT` (default desligado, ver docs/ERROR-HANDLING.md). `externalStatus` guarda o código numérico bruto do Bling; `status` é a versão simplificada (pending/issued/cancelled/error) espelhada pro PDV local via `GET /fiscal/sale/:saleId` (guardado por `TerminalApiKeyGuard`) |

## Evento central

`SaleConfirmed` é o gatilho para: débito de estoque (mesma transação local), auditoria (mesma transação local, ainda não implementada), impressão do comprovante não-fiscal (**Sprint 11**, ESC/POS via hardware do Electron), e gravação no SyncOutbox local (Sprint 6) → Intermediador → **Bling (Adapter real desde a Sprint 7)** → emissão fiscal (**Sprint 12**, opt-in via `BLING_NFCE_AUTO_EMIT`).
